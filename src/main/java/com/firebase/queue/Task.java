...
public class Task {
    interface Listener {
        ...
        void onFailure(String error, boolean canRetry);
    }
    ...
    public Task(DatabaseReference taskRef, String ownerId, Map<String, Object> data, TaskSpec taskSpec, TaskReset taskReset, ValidityChecker validityChecker, boolean suppressStack) {
    ...
    void process(TaskProcessor taskProcessor) {
        ...
        processingThreadRef = new WeakReference<>(Thread.currentThread());
    ...
    public void resolve() {
        resolve(new HashMap<>(), null);
    }

    public void resolve(Listener listener) {
        resolve(new HashMap<>(), listener);
    }

    public void resolve(Map<String, Object> newTask) {
        resolve(newTask, null);
    }

    public void resolve(final Map<String, Object> newTask, final Listener listener) {
        resolve(newTask, listener, 0);
    }

    private void resolve(final Map<String, Object> newTask, final Listener listener, final long retries) {
    ...
    public void reject(String error, Listener listener) {
        internalReject(error, listener);
    }

    private void internalReject(final Object errorObject, final Listener listener) {
        internalReject(errorObject, listener, 0);
    }

    private void internalReject(final Object errorObject, final Listener listener, final long retries) {
    ...
    private boolean canActionBeTakenBeforeTransaction(String action, Listener listener) {
        synchronized (actionLock) {
            if (!canTakeAction()) {
    ...
    private void onActionCouldNotBeTaken(String action, String actionError, Listener listener) {
        cancel();
    ...
    private boolean isTaskStillValid(String action, Listener listener) {
    ...
    private void onTransactionSuccess(boolean committed, String taskKey, String action, Listener listener) {
        if (committed) {
            complete();
